...
async def get_all(db: AsyncSession) -> list[models_template.TemplateItem]:
    result = await db.execute(select(models_template.TemplateItem))
    return list(result.scalars().all())


async def get_by_id(db: AsyncSession, item_id: int) -> models_template.TemplateItem | None:
    result = await db.execute(
        select(models_template.TemplateItem).where(models_template.TemplateItem.id == item_id)
    )
